...
export class TaxonDescriptionBlockFindAllParams extends BaseFindAllParams {
    ...
    @Min(0)
    @Max(TaxonDescriptionBlockFindAllParams.MAX_LIMIT)
    limit: number = TaxonDescriptionBlockFindAllParams.DEFAULT_LIMIT;

    @ApiProperty({ required: false, default: TaxonDescriptionBlockFindAllParams.DEFAULT_OFFSET })
    @Min(0)
    offset: number = TaxonDescriptionBlockFindAllParams.DEFAULT_OFFSET;
}
